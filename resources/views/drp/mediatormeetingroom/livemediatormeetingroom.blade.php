                    ...
                    <div class="card-header">
                        <div class="row flex-between-end">
                            <div class="col-auto align-self-center">
                                <h5 class="mb-0" data-anchor="data-anchor">Mediator Meeting Room - Live</h5>
                            </div>
                    ...
                    <div class="card-body px-0 pb-0 table-meetinglist">
                        <div class="row gy-3">
                            ...
                            <div class="col-lg-12 col-12">
                                <form id="sendnoticeForm" action="{{ route('drp.mediatormeetingroom.savenotice') }}" method="POST">
                                    @csrf
...
@section('js')
    ...
    <script>
        ...
        try {
            ...
            //     try {
            //         const response = await fetch("{{ route('drp.mediatormeetingroom.saveRecording') }}", {
            //             method: "POST",
            //             headers: {
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $('#caseSelector').on('change', function() {
                ...
                // Fetch the flattened data dynamically
                $.ajax({
                    url: "{{ route('drp.mediatormeetingroom.getFlattenedCaseData', ':caseId') }}".replace(
                        ':caseId', caseId),
                    method: 'GET',
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#caseSelector').on('change', function() {
                ...
                // Fetch Notices
                $.ajax({
                    url: "{{ route('drp.mediatormeetingroom.fetch.notices') }}",
                    method: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        case_id: caseId
                ...
                // Fetch Awards
                $.ajax({
                    url: "{{ route('drp.mediatormeetingroom.fetch.awards') }}",
                    method: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        case_id: caseId
                ...
                $.ajax({
                    url: "{{ route('drp.mediatormeetingroom.fetch.settlementagreements') }}",
                    method: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        case_id: caseId
    ...
    <script>
        $(document).ready(function() {
            $('#sendnoticeForm').on('submit', function(e) {
                ...
                $.ajax({
                    url: "{{ route('drp.mediatormeetingroom.savenotice') }}",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
        ...
        $(document).on('click', '#endMeetingRoom', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('drp.mediatormeetingroom.close') }}",
                        method: "POST",
                        data: {
                            _token: "{{ csrf_token() }}",
                            room_id: roomId
                            ...
                            if (response.success) {
                                Swal.fire({
                                    ...
                                    window.location.href =
                                        "{{ route('drp.mediatormeetingroom.mediatormeetingroomlist') }}?success=1";
                                });
                            } else {
