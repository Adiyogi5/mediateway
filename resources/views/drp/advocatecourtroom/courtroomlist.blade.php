                                        ...
                                        {{-- <tbody>
                                            @forelse ($upcomingRooms as $room)
                                                <tr class="bg-blue">
                                                    ...
                                                    <td class="pt-3">
                                                        @if($room->status == 1)
                                                            <a href="{{ route('drp.advocatecourtroom.livecourtroom', $room->room_id) }}?case_ids={{ $room->case_ids }}"
    ...
    <script>
        $(document).ready(function () {
            ...
            const upcomingTable = $('#upcomingTable').DataTable({
                ajax: '{{ route("drp.advocatecourtroom.datatable.upcoming") }}',
                columns: [
                    { data: 'case_numbers' },
                    { data: 'hearing_type' },
            ...
            const closedTable = $('#closedTable').DataTable({
                ajax: '{{ route("drp.advocatecourtroom.datatable.closed") }}',
                columns: [
                    { data: 'case_numbers' },
                    { data: 'hearing_type' },
                    { data: 'individual_name' },
