...
@section('content')
    <div class="card mb-3">
        ...
        <div class="card-body row">
            <form class="row" action="{{ route('caseassign.updatecasedetail', $case->id) }}" id="editCase"
                ...
                <div class="col-md-6 col-12 mb-3">
                    ...
                    <select id="case_type" name="case_type" class="form-select">
                        @foreach (config('constant.case_type') as $key => $caseType)
                            <option value="{{ $key }}" {{ $case->case_type == $key ? 'selected' : '' }}>
                                {{ $caseType }}</option>
                        @endforeach
                    </select>
                    @error('case_type')
                ...
                <div class="col-md-6 col-12 mb-3">
                    ...
                    <select id="status" name="status" class="form-select" required>
                        <option value="1" {{ $case->status == 'Active' ? 'selected' : '' }}>Active</option>
                        <option value="0" {{ $case->status == 'InActive' ? 'selected' : '' }}>InActive</option>
                    </select>
                </div>
                ...
                @foreach ($documents as $key => $label)
                    <div class="col-md-6 col-12 mb-3 document-upload" id="upload-{{ $key }}">
                        ...
                        <input class="form-control" type="file" id="{{ $key }}" name="{{ $key }}" />
