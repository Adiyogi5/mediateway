...
class CreateLiveEmailMediatorMeetingRoom extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'bulk:create-live-email-mediator-meeting-room';
    ...
    public function handle()
    {
        try {
            // ###############################################
            // Create Live Mediation Meeting Room - Send Email
            // ###############################################
            ...
            foreach ($meetingroomData as $meetingRoom) {
                ...
                foreach ($cases as $case) {
                    ...
                    $subject = 'Mediation Meeting Link';
                    $case_id = $case->id;
                    ...
                    try {
                        ...
                        Log::info("Mediation Meeting Room Email sent successfully for FileCase ID: {$case_id}");
                    } catch (\Exception $e) {
                        Log::warning("Mediation Meeting Room Email failed for FileCase ID: {$case_id}. Error: " . $e->getMessage());
                        $allEmailsSent = false;
                    }
                }
