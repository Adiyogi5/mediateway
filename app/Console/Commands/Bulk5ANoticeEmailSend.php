...
class Bulk5ANoticeEmailSend extends Command
{
    ...
    public function handle()
    {
        ...
        // Join only the 10-type notices to get their details (leftJoin to include cases even when 10-type doesn't exist)
            ...
            ->leftJoin('notices as notice12', function ($join) {
                $join->on('notice12.file_case_id', '=', 'file_cases.id')
                    ->where('notice12.notice_type', 12);
            })
        ...
        // NEW condition: must also have notice_type = 12
            ->whereHas('notices', function ($q) {
                $q->where('notice_type', 12);
            })
                ...
                if (! empty($assigncaseData)) {
                    ...
                    if (empty($value->notice10)) {
                        ...
                        $data = [
                            ...
                            'STAGE 4A NOTICE DATE'                          => $value->file_case_details->stage_4a_notice_date ?? '',
                            'STAGE 5A NOTICE DATE'                          => now()->format('d-m-Y'),
                        ];
                            ...
                            if ($validator->fails()) {
                                ...
                                try {
                                    Mail::send('emails.simple', compact('subject', 'description'), function ($message) use ($value, $subject, $email) {
                                        ...
                                        //     'mime' => 'application/pdf',
                                        // ]);
                                            ->attach(public_path(str_replace('\\', '/', 'storage/' . $value->notice10)), [
                                                'mime' => 'application/pdf',
                                            ]);
                                    });
