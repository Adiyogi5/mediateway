...
class CreateLiveWhatsappConciliatorMeetingRoom extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'bulk:create-live-whatsapp-conciliator-meeting-room';
    ...
    public function handle()
    {
        try {
            // #####################################################
            // Create Live Conciliation Meeting Room - Send Whatsapp
            // #####################################################
            ...
            foreach ($meetingroomData as $meetingRoom) {
                ...
                foreach ($cases as $case) {
                    ...
                    $message    = route('front.guest.liveconciliatormeetingroom', ['room_id' => $room_id]) . "?case_id=$case_id&message=$encodedMessage";
                    ...
                    try {
                        ...
                        if ($response->successful() && isset($responseData['status']) && $responseData['status'] == 1) {
                            Log::info("Conciliation Meeting Room WhatsApp sent successfully for FileCase ID: {$case_id}");
                        } else {
            ...
            Log::error("Error creating conciliation meetingroom entry: " . $th->getMessage());
        }
    }
}
