...
class Bulk3CNoticeSend extends Command
{
    ...
    {
        // ##################################################
        // In the matter of an Arbitration before the Sole Arbitrator - 3B - Notice Send
        // ##################################################
        $caseData = FileCase::with('file_case_details')
            ...
            ->whereHas('notices', function ($query) {
                $query->where('notice_type', 1)
                    ->whereRaw('DATEDIFF(CURDATE(), notices.notice_date) >= organization_notice_timelines.notice_3c');
            })
        ...
        foreach ($caseData as $key => $value) {
            try {
                ...
                if (($assigncaseData->confirm_to_arbitrator == 0)) {
                    ...
                    $data = [
                        ...
                        'STAGE 3-B NOTICE: APPOINTMENT OF ARBITRATOR'                     => now()->format('d-m-Y'),
                        'STAGE 3-C NOTICE: ACCEPTANCE AND DISCLOSURE'                     => now()->format('d-m-Y'),
                    ];
