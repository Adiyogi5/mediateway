...
class ConciliatorMeetingRoomController extends Controller
{
    ...
    public function index(Request $request): View | JsonResponse | RedirectResponse
    {
        ...
        $conciliatormeetingroomLiveUpcoming = ConciliatorMeetingRoom::select(
                'conciliator_meeting_rooms.*',
            ...
            ->leftJoin('drps', 'drps.id', '=', 'conciliator_meeting_rooms.conciliator_id')
            ->leftJoin('file_cases', function ($join) {
                $join->On(DB::raw('FIND_IN_SET(file_cases.id, conciliator_meeting_rooms.meeting_room_case_id)'), '>', DB::raw('0'));
            })
            ->where('conciliator_meeting_rooms.conciliator_id', $drp->id)
            ->where(function ($query) {
                $query->where('conciliator_meeting_rooms.date', '>', Carbon::today()->toDateString())
                    ->orWhere(function ($subQuery) {
                        $subQuery->where('conciliator_meeting_rooms.date', Carbon::today()->toDateString())
                                ->where('conciliator_meeting_rooms.time', '>=', Carbon::now()->format('H:i:s'));
                    })
                    ->orWhere(function ($subQuery) {
                        $subQuery->where('conciliator_meeting_rooms.date', Carbon::today()->toDateString())
                                ->where('conciliator_meeting_rooms.status', 1);
                    });
            })
            ->groupBy(
                    'conciliator_meeting_rooms.id', 
                    'conciliator_meeting_rooms.meeting_room_case_id',
                    'conciliator_meeting_rooms.room_id',
                    'conciliator_meeting_rooms.date',
                    'conciliator_meeting_rooms.time',
                    'conciliator_meeting_rooms.conciliator_id',
                    'conciliator_meeting_rooms.recording_url',
                    'conciliator_meeting_rooms.send_mail_to_respondent',
                    'conciliator_meeting_rooms.email_send_date',
                    'conciliator_meeting_rooms.send_whatsapp_to_respondent',
                    'conciliator_meeting_rooms.whatsapp_dispatch_datetime',
                    'conciliator_meeting_rooms.status',
                    'conciliator_meeting_rooms.deleted_at',
                    'conciliator_meeting_rooms.created_at',
                    'conciliator_meeting_rooms.updated_at',
                    'drps.name'
                )
        ->get();

        $conciliatormeetingroomLiveClosed = ConciliatorMeetingRoom::select(
                'conciliator_meeting_rooms.*',
                'drps.name as conciliator_name',
                DB::raw('GROUP_CONCAT(DISTINCT file_cases.case_number SEPARATOR ", ") as case_numbers'),
                DB::raw('GROUP_CONCAT(DISTINCT file_cases.id SEPARATOR ", ") as case_ids')
            ...
            ->where('conciliator_meeting_rooms.conciliator_id', $drp->id)
            ->where('conciliator_meeting_rooms.status', 0)
            ->where(function ($query) {
                $query->where('conciliator_meeting_rooms.date', '<', Carbon::today()->toDateString())
                    ->orWhere(function ($subQuery) {
                        $subQuery->where('conciliator_meeting_rooms.date', Carbon::today()->toDateString())
        ...
        $upcomingroomCount = $conciliatormeetingroomLiveUpcoming->count();
        $closedroomCount = $conciliatormeetingroomLiveClosed->count();

        $upcomingRooms = $conciliatormeetingroomLiveUpcoming;
        $closedRooms = $conciliatormeetingroomLiveClosed;

        return view('drp.conciliatormeetingroom.conciliatormeetingroomlist', compact('drp','title','upcomingRooms','closedRooms','upcomingroomCount','closedroomCount'));
    }
    ...
    public function upcomingRoomsData()
    {
        ...
        $upcomingRooms = ConciliatorMeetingRoom::select(
                'conciliator_meeting_rooms.*',
                'drps.name as conciliator_name',
                DB::raw('GROUP_CONCAT(DISTINCT file_cases.case_number SEPARATOR ", ") as case_numbers'),
                DB::raw('GROUP_CONCAT(DISTINCT file_cases.id SEPARATOR ", ") as case_ids')
        ...
        return datatables()->of($upcomingRooms)
            ...
            ->addColumn('action', function ($room) {
                if ($room->status == 1) {
                    return '<a href="' . route('drp.conciliatormeetingroom.liveconciliatormeetingroom', $room->room_id) . '?case_ids=' . $room->case_ids . '" class="fa fa-video btn bg-success text-white fs-6"></a>';
                } else {
    ...
    public function closedRoomsData()
    {
        ...
        $closedRooms = ConciliatorMeetingRoom::select(
                'conciliator_meeting_rooms.*',
                'drps.name as conciliator_name',
                DB::raw('GROUP_CONCAT(DISTINCT file_cases.case_number SEPARATOR ", ") as case_numbers'),
                DB::raw('GROUP_CONCAT(DISTINCT file_cases.id SEPARATOR ", ") as case_ids')
    ...
    public function liveconciliatormeetingroom(Request $request, $room_id): View | JsonResponse | RedirectResponse
    {
        ...
        return view('drp.conciliatormeetingroom.liveconciliatormeetingroom', compact('drp',
            'title',
            'caseData',
            'orderSheetTemplates', 
    ...
    public function saveRecording(Request $request)
    {
        ...
        // Locate the conciliatormeetingroom based on room ID
        $conciliatormeetingroom = ConciliatorMeetingRoom::where('room_id', $validated['room_id'])->first();

        if ($conciliatormeetingroom) {
            ...
            $conciliatormeetingroom->recording_url = $recordingPath;
            $conciliatormeetingroom->save();
    ...
    public function closeconciliatormeetingroom(Request $request)
    {
        $roomId = $request->room_id;

        $conciliatormeetingroom = ConciliatorMeetingRoom::where('room_id', $roomId)->first();

        if ($conciliatormeetingroom) {
            $conciliatormeetingroom->status = 0; // Change status to 0
            $conciliatormeetingroom->save();
