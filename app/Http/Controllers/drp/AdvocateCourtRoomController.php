...
class AdvocateCourtRoomController extends Controller
{
    ...
    public function index(Request $request): View | JsonResponse | RedirectResponse
    {
        ...
        $courtRoomLiveUpcoming = CourtRoom::select(
            ...
            ->leftJoin('drps', 'drps.id', '=', 'court_rooms.advocate_id')
            ...
            ->where('court_rooms.advocate_id', $drp->id)
        ...
        return view('drp.advocatecourtroom.courtroomlist', compact('drp','title','upcomingRooms','closedRooms','upcomingroomCount','closedroomCount'));
    }
    ...
    public function upcomingRoomsData()
    {
        ...
        return datatables()->of($upcomingRooms)
            ...
            ->addColumn('action', function ($room) {
                if ($room->status == 1) {
                    return '<a href="' . route('drp.advocatecourtroom.livecourtroom', $room->room_id) . '?case_ids=' . $room->case_ids . '" class="fa fa-video btn bg-success text-white fs-6"></a>';
                } else {
    ...
    public function livecourtroom(Request $request, $room_id): View | JsonResponse | RedirectResponse
    {
        ...
        $caseData = FileCase::select('file_cases.*', 'drps.name as advocate_name')
            ->with(['file_case_details', 'guarantors'])
            ->join('assign_cases', 'assign_cases.case_id', '=', 'file_cases.id')
            ->join('drps', 'drps.id', '=', 'assign_cases.advocate_id')
            ->whereIn('file_cases.id', $caseIds) // Use whereIn instead of find()
            ->get();
       
        $flattenedCaseData = $this->flattenAdvocateCaseData($caseData);
        ...
        //ZegoCloud Service---------------------
        $localUserID = $drp->slug; // advocate
        $remoteUserID = $caseIds; // Individual
        $roomID = $room_id;
     
        $zegoToken = $this->generateZegoToken($localUserID, $drp->name);

        return view('drp.advocatecourtroom.livecourtroom', compact(
            'drp',
            'title',
            'caseData',
            'orderSheetTemplates',
    ...
    public function getFlattenedAdvocateCaseData($caseId)
    {
        $caseData = FileCase::select('file_cases.*', 'drps.name as advocate_name')
            ->with(['file_case_details', 'guarantors'])
            ->join('assign_cases', 'assign_cases.case_id', '=', 'file_cases.id')
        ...
        if ($caseData) {
            $flattenedCaseData = $this->flattenAdvocateCaseData(collect([$caseData]));
    ...
    function flattenAdvocateCaseData($caseData) {
        $flat = [];
